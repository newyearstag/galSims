...
(R_gal_sim,x_sim,y_sim,z_sim,mass_sim,N_ly_sim,age_sim,rad_sim,l_sim,vR_sim,regNum_sim,b_sim,flux_sim,dist_sim) = (list(),list(),list(),list(),list(),list(),list(),list(),list(),list(),list(),list(),list(),list())
indexS = 0
while indexS < len(csvFileS) :
    ...
    if (long>northMin) and (long<northMax):
        ...
        rad_sim.append(float(csvFileS[indexS][7]))
        l_sim.append(long)
        ...
        dist_sim.append(float(pow(x**2+y**2+z**2,.5)))
    indexS += 1
...
## R_Gal Histogram ##
n_bins = 100
ax0.hist(R_gal_sim, n_bins, normed=1, range=[0,16], histtype='bar', color = sim_color, label = '')
ax0.hist(R_gal_wise, n_bins, normed=1, range=[0,16], histtype='step', color = wise_color, label = '')
#ax0.legend(prop={'size': 10})
ax0.set_title('Galactocentric Radius (kpc)')



## Heliocentric Distance ##
n_bins = 100
ax1.hist(dist_sim, n_bins, normed=1, range=[0,20], histtype='bar', color = sim_color, label = '')
ax1.hist(dist_wise, n_bins, normed=1, histtype='step', color = wise_color, label = '')
ax1.set_title('Heliocentric Distance (kpc)')
